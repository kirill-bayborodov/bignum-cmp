/**
 * @file    bench_bignum_cmp.c
 * @brief   Микробенчмарк для профилирования bignum_cmp.
 * @author  git@bayborodov.com
 * @version 1.0.0
 * @date    20.10.2025
 *
 * @details
 *   Вызывает функцию bignum_cmp на случайных
 ...
 *    benchmarks/bench_bignum_cmp.c build/bignum_cmp.o \
 *    -o bin/bench_bignum_cmp
 *
 * # Запуск perf с записью стека через frame-pointer
 * /usr/local/bin/perf record -F 9999 -o benchmarks/reports/report_bench_bignum_cmp -g -- bin/bench_bignum_cmp
 *
 * # Отчёт, отфильтрованный по символу
 * /usr/local/bin/perf report -i benchmarks/reports/report_bench_bignum_cmp --stdio --symbol-filter=bignum_cmp
 */
